package camera

import (
	"bytes"
	"encoding/binary"
	"errors"
	"os"

	"github.com/korandiz/v4l"
	"github.com/korandiz/v4l/fmt/mjpeg"
	"github.com/korandiz/v4l/fmt/yuyv"
)

const jpegAccelEnabled = false

const cameraDevPath = "/dev/video0"
const cameraWidth = 640
const cameraHeight = 480
const CameraFPS = 10

const jpegDevPath = "/dev/team4_jpeg"

// We use a global here, because we have a fixed hardware setup.
var camera *v4l.Device
var jpegCompressor *os.File

func Open() {
	cam, err := v4l.Open(cameraDevPath)
	camera = cam
	if err != nil {
		panic(err)
	}

	var format uint32
	if jpegAccelEnabled {
		format = yuyv.FourCC
	} else {
		format = mjpeg.FourCC
	}

	camera.SetConfig(v4l.DeviceConfig{
		Format: format,
		Width:  cameraWidth,
		Height: cameraHeight,
		FPS:    v4l.Frac{N: CameraFPS, D: 1},
	})

	// Necessary because our JPEG compressor supports only
	// grayscale.
	err = setControl("Saturation", 0)
	if err != nil {
		panic(err)
	}

	if jpegAccelEnabled {
		jpegCompressor, err = os.Open(jpegDevPath)
		if err != nil {
			panic(err)
		}
	}
}

func setControl(name string, value int32) error {
	controls, err := camera.ListControls()
	if err != nil {
		return err
	}

	var cid *uint32
	for _, control := range controls {
		if control.Name == name {
			cid = &control.CID
			break
		}
	}

	if cid == nil {
		return errors.New("Couldn't find control on camera.")
	}

	err = camera.SetControl(*cid, value)
	return err
}

func Close() {
	if jpegAccelEnabled {
		jpegCompressor.Close()
	}

	camera.Close()
}

func Start() {
	camera.TurnOn()
}

func Stop() {
	camera.TurnOff()
}

func Capture() ([]byte, error) {
	buf, err := camera.Capture()
	if err != nil {
		return nil, err
	}

	dst := new(bytes.Buffer)
	dst.ReadFrom(buf)

	if !jpegAccelEnabled {
		return dst.Bytes(), nil
	}

	return compressImage(dst.Bytes()), nil
}

// Compress a YUYV 4:2:2 image as a grayscale JPEG, without headers.
func compressScan(raw []byte) []byte {
	buf := new(bytes.Buffer)

	binary.Write(buf, binary.LittleEndian, cameraWidth)
	binary.Write(buf, binary.LittleEndian, cameraHeight)
	buf.Write(raw)

	// The way the kernel driver works, we have to do one write
	// syscall for one transaction.
	jpegCompressor.Write(buf.Bytes())

	// This blocks until the accelerator is finished.
	var result []byte
	jpegCompressor.Read(result)

	return result
}

// Compress as before, but add the appropriate JPEG headers.
func compressImage(raw []byte) []byte {
	buf := new(bytes.Buffer)
	buf.Write(jpeg_header)
	buf.Write(compressScan(raw))
	buf.Write(jpeg_footer)
	return buf.Bytes()
}

// Contains the quantization table, huffman table, and width/height information.
var jpeg_header = []byte{
	,
	(cameraHeight & 0xff00) >> 8, cameraHeight & 0xff,
	(cameraWidth & 0xff00) >> 8, cameraWidth & 0xff,
	0x01, 0x01, 0x11, 0x00, 0xFF, 0xDA, 0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x3F, 0x00,
}

var jpeg_footer = []byte{
	0xFF, 0xD9,
}
